...
void reset(){   // sends the normal state for the LCD display
    commSend(0x01);
    dataSend(0x52);
    dataSend(0x65);
    dataSend(0x73);
...
__interrupt void EUSCI_B0_I2C_ISR(void){
    ...
    if(Data == 0x2A){   //Catches the reset code and resets the LCD to the reset case
        reset();
    }else{
        //Starts LCD back at beginning if already holds data
        if(count == 13){
            count=1;
            commSend(0x84); //address for resolution number
            dataSend(Data);
        //moves LCD to LM19 temperature display
        }else if(count == 1){
            commSend(0x8A);
            count++;
            dataSend(Data);
        }else if(count == 5){//moves LCD to mode letter
            commSend(0xC0);
            count++;
            dataSend(Data);
        }else if(count == 6){//moves LCD to seconds
            commSend(0xC2);
            count++;
            dataSend(Data);
        }else if(count == 9){//moves LCD to temperature lm92
            commSend(0xCA);
            count++;
            dataSend(Data);
        }
        else{              //sends data and increments counter
            dataSend(Data);
            count++;
        }
    }
